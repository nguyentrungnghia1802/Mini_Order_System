...
public sealed class OutboxDispatcher : BackgroundService
{
    ...
    public async Task<bool> DispatchOnceAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            await transport.PublishAsync(message, claim.TraceParent, cancellationToken);
        ...
        {
            OutboxLog.PublishFailed(_logger, exception, claim.MessageId, claim.AttemptCount);
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            OutboxLog.PublishLeaseLost(_logger, claim.MessageId, _workerId);
            return;
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!message.TryMarkFailed(
                ...
                deadLetter))
        {
            OutboxLog.FailedPublishLeaseLost(_logger, claim.MessageId);
            return;
        }
...
internal static partial class OutboxLog
{
    [LoggerMessage(
        ...
        Message = "Order outbox message {MessageId} failed on attempt {AttemptCount}.")]
    ...
    [LoggerMessage(
        ...
        Message = "Order outbox message {MessageId} was published but its lease was no longer owned by worker {WorkerId}.")]
    public static partial void PublishLeaseLost(ILogger logger, Guid messageId, string workerId);
