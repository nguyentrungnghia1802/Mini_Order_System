...
- Security/safety: tracked-secret checks, package/image audit, backup/restore drills, and `git diff --check` pass. The retained-volume pagination fix was pushed on `codex/fix-e2e-persistent-product-list` through `29525ca` and fast-forward merged into `main`; final remote SHA is verified during the current delivery.
