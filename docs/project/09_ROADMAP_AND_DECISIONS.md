...
Phase 5 status: contract, transport, direct-publish, Notification persistence, and the `OrderConfirmedV1` consumer foundations are implemented. `MicroShop.Contracts.Orders.OrderConfirmedV1` is implemented with message/order IDs, customer destination, totals, item snapshots, UTC occurrence time, schema version, and serialization tests. MassTransit RabbitMQ options, bounded retry, durable Notification endpoint, environment credentials, readiness registration, and post-commit Order publish with trace/correlation propagation are configured. Notification owns `consumed_messages` and `notifications`, applies its migration, persists both records transactionally, and suppresses duplicate message IDs. The Notification read API/UI, broker-level recovery exercises, and full Compose validation remain incomplete; the direct publish dual-write gap is intentionally a Phase 7 outbox task.
