...
The target topology below is implemented incrementally. The repository now contains the Phase 0 foundation, the Product Service catalog/update plus Product-owned reservation slice, the Order Service persistence/native API plus typed Product reservation and cancellation orchestration, a tested YARP Gateway route boundary, and the Notification persistence/consumer/read API plus Angular screen slices. Product owns its EF Core model/migrations, service-native catalog endpoints, and internal reservation/release endpoints; Order owns its domain model, state history, separate EF Core migration, native create/list/detail/cancel API, typed HTTP client, and explicit distributed outcome states; Notification owns its database, migration, consumer idempotency record, and notification read model. RabbitMQ broker recovery is verified with container-backed integration tests; full application-container Compose validation and end-to-end behavior remain phase-scoped work.
